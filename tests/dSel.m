...
m = 10; n = 50; k = 2; % m rows, n cols, k nonzeros.
...
fprintf('one norm of standard: %5.3f, one-norm of GN: %5.3f\n', norm(xLS, 1), norm(xGN, 1));

fprintf('norm diff between CVX and Dantz: %5.3f, between CVX and Stand: %5.3f \n', norm(xGN - xCVX, inf),norm(xLS - xCVX, inf));


%% Plot the results

figure(1)
plot(1:n, x0 + 2, 1:n, xLS(1:n), 1:n, xGN(1:n) - 2, 1:n, xCVX(1:n) - 4,   '-k', 'Linewidth', 1.5);

legend('true', 'Standard', 'Dantzig', 'CVX');

figure(2);
plot(1:m, b - A*x0 + 1.5, 1:m, b - A*xLS(1:n), 1:m, b - A*xGN(1:n) - 1.5, 1:m, b - A*xCVX(1:n) - 3,   '-k', 'Linewidth', 2); 
legend('true', 'Standard', 'Dantzig', 'CVX');
